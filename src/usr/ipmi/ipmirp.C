...
void IpmiRP::idle(void)
{
    ...
    for (IPMI::send_q_t::iterator i = iv_sendq.begin(); i != iv_sendq.end();)
    {
        ...
        IPMI_TRAC("transmitting %x:%x, queue length %d",
    ...
    do
    {
        ...
        if (itr == iv_respondq.end())
        {
            ...
            IPMI_TRAC("IPMI command response, but no waiter: seq %x %x:%x",
                      rcv_buf->iv_seq, rcv_buf->iv_netfun, rcv_buf->iv_cmd);
            IPMI_TRAC("deleting response data %x:%x, no waiter",
                      rcv_buf->iv_netfun, rcv_buf->iv_cmd);
...
namespace IPMI
{
                        ...
                        size_t& io_len, uint8_t*& io_data)
    {
        ...
        IPMI_TRAC("sync send %x:%x", i_netfun, i_cmd);
        ...
        IPMI_TRAC("back from msg_sendrecv %d", rc);
        ...
        else
        {
            ...
            IPMI_TRAC("deleting data %x:%x .", i_netfun, i_cmd);
            delete[] io_data;
        }
                    ...
                    const size_t i_len, uint8_t* i_data)
    {
        ...
        IPMI::Message* ipmi_msg = IPMI::Message::factory(i_netfun, i_cmd,
                                                         i_len, i_data,
                                                         IPMI::TYPE_ASYNC);
        IPMI_TRAC("async send %x:%x", i_netfun, i_cmd);
