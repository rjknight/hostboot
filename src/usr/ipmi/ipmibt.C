...
namespace IPMI
{
    ...
    errlHndl_t BTMessage::xmit(void)
    {
        ...
        {
            IPMI_TRAC("deleting data %x:%x .", iv_netfun, iv_cmd);
            delete[] iv_data;
        }
    ...
    errlHndl_t BTMessage::recv(void)
    {
        ...
        if (err)
        {
            IPMI_TRAC("deleting data %x:%x, driver error", iv_netfun, iv_cmd);
            delete[] iv_data;
    ...
    bool BTSyncMessage::xmit(respond_q_t& i_respondq)
    {
        ...
        if (err)
        {
            ...
            if (msg_respond(mq, iv_msg))
            {
                ...
                IPMI_TRAC("msg_respond() i/o error (transmit)");
    ...
    bool BTAsyncMessage::xmit(respond_q_t&)
    {
        ...
        else if (iv_state != EAGAIN)
        {
            ...
            IPMI_TRAC("deleting msg %x:%x .", iv_netfun, iv_cmd);
            delete this;
        }
