 ...
 * @param[in]   i_target        IPMI target, ignored use the master sentinal
 * @param[in]   i_buffer        Pointer to byte to read into
 ...
 * @param[in]   i_target        IPMI target, ignored use the master sentinal
 * @param[out]  o_buffer        Pointer to byte for input
...
{
    IPMI_TRAC("poll_control_register> " );
...
void IpmiDD::pollCtrl(void)
{
    ...
    while(1)
    {
        ...
        if (err)
        {
            IPMI_TRAC("polling loop encountered an error, exiting");
        ...
        else
        {
            ...
            {
                IPMI_TRAC("sending state response");
                msg = msg_allocate();
            ...
            {
                IPMI_TRAC("sending state sms/event");
                msg = msg_allocate();
...
inline errlHndl_t IpmiDD::reset(void)
{
    ...
    {
        IPMI_TRAC("reset: receiving ...", ctrl);
        err = receive(&msg);
    ...
    IPMI_TRAC("done resetting the IPMI BT interface");
...
errlHndl_t IpmiDD::send(IPMI::BTMessage* i_msg)
{
    ...
    do
    {
        ...
        {
            IPMI_TRAC("interface busy 0x%x & 0x%x, EAGAIN", ctrl, IDLE_STATE);
        ...
        for( size_t i = 0; (i < i_msg->iv_len) && (err == NULL); ++i)
        {
            ...
            IPMI_TRAC("data: %x", i_msg->iv_data[i]);
        }
        if (err) { break; }

        // If all is well, alert the host we sent bits.
        IPMI_TRAC("writing CTRL_H2B_ATN");
...
errlHndl_t IpmiDD::receive(IPMI::BTMessage* o_msg)
{
    ...
    do
    {
        err = readLPC(REG_CONTROL, ctrl);
        if (err) { break; }
        IPMI_TRAC("receive ctrl register 0x%x %s", ctrl,
                  (ctrl & CTRL_B2H_ATN) ? "B2H" : "SMS");

        // Tell the interface we're busy.
        IPMI_TRAC("writing CTRL_H_BUSY");
        ...
        IPMI_TRAC("read header (ctrl %x) len %x seq %x %x:%x cc %x",
                  ctrl, o_msg->iv_len, o_msg->iv_seq,
                  o_msg->iv_netfun, o_msg->iv_cmd, o_msg->iv_cc);
        ...
        for( size_t i = 0; (i < o_msg->iv_len) && (err == NULL); ++i)
        {
            ...
            IPMI_TRAC("data: %x", o_msg->iv_data[i]);
        }
        if (err) { break; }
    ...
    if (marked_busy)
    {
        ...
        IPMI_TRAC("writing (to clear) CTRL_H_BUSY");
    ...
    iv_eagains(false)
{
    ...
    if (err)
    {
        IPMI_TRAC("error resetting the BT interface");
